...
public class OrderController {
    ...
    private final GetOrderUseCase getOrderList;
    private final ValidateOrderUseCase validateOrder;
    @PostMapping(value = "/order")
    public @ResponseBody ResponseEntity order(@RequestBody @Valid ItemCommand itemCommand
        ...
        try {
            orderId = doOrderUseCase.order(email, itemCommand.toRequest());
        } catch(Exception e){
    ...
    public String orderHist(@PathVariable("page") Optional<Integer> page, Principal principal, Model model){
        ...
        Page<OrderHistDto> ordersHistDtoList = getOrderList.getOrderList(principal.getName(), pageable);
    ...
    public @ResponseBody ResponseEntity cancelOrder(@PathVariable("orderId") Long orderId , Principal principal){
        ...
        if(!validateOrder.validateOrder(orderId, principal.getName())){
            return new ResponseEntity<String>("주문 취소 권한이 없습니다.", HttpStatus.FORBIDDEN);
        }

        doOrderUseCase.cancelOrder(orderId);
        return new ResponseEntity<Long>(orderId, HttpStatus.OK);
    }
}
